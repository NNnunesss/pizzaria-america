...
// A seguir annotation do (Spring Wem MVC) dependencia: "spring-boot-starter-webmvc" (pon.xml)
...
// @DeletMapping: Utilizado para "excluir" dados na API
// @PatchMapping: Utilizado para "atualizar parcialmente" dados na API, exemplo: mudar o status de um produto

// ResponseEntity: Controla a resposta HTTP completa de uma API, permitindo definir o copo (body), o codigo de status (200, 201, 400, etc...)
//                 e os caeçcalhos (headers)      
@RestController
   ...
   try {
    Long idLong = Long.parseLong(id);
      ...
      if(produtoBanco == null) {
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Produto com o id " + id + " nao encontrado");
      }
     Produto produAtualizado = produtoService.atualizar(idLong, produto);
  ...
  public ResponseEntity <?> delete(@PathVariable String id) {
   try {
    ...
    if(produtoBanco == null){
     ...
     if (excluido) {
      return ResponseEntity.ok("Produto com o id"+ id +"excluido com sucesso.");
     }else{
      return ResponseEntity.ok("Nao foi possivel excluir o produto com o id "+ id);
     }
